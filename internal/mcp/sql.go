...
func (SQLInput) Schema() *jsonschema.Schema {
	...
	schema.Properties["visualize"].Description = "Render the results in the local web UI instead of (or in addition to) returning them as text. 'table' shows the rows in a table; 'chart' renders a chart (provide chart_config). When set, the query runs in the browser and the live UI is updated so the user sees exactly what you ran. Opens a browser if one isn't already connected. Omit to just run server-side and return rows as text."
	schema.Properties["visualize"].Enum = []any{"table", "chart"}
	schema.Properties["chart_config"].Description = "JavaScript source defining a function `chart(data)` that returns an Apache ECharts option object. `data` provides `data.rows` (array of row objects keyed by column name) and `data.columns` ([{name, type}]). Only used with visualize='chart'. Applied to the chart and shown in the UI's config editor (overwriting any existing config). When set, the response includes a PNG image of the rendered chart so you can verify its appearance."
	return schema
}
...
// summary (capped rows + columns) plus, for chart view, a rendered image.
