...
- **Two serving modes**: the regular `ghost mcp start` (authoring) registers the `ghost_mcp_tool_refresh` management tool plus a startup snapshot of every database's function tools, introspected concurrently before the server starts serving; `ghost mcp start --serve <database_ref>` (consumer) exposes *only* that database's generated tools; unlike the rest of the feature, the `--serve` flag is not gated behind `GHOST_EXPERIMENTAL`. Capability enumeration (`ghost mcp list`/`get`) constructs the server without `Options.FunctionTools`, so it registers the management tool but never connects to any databases.
