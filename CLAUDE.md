...
- **Catalog introspection, no SQL parsing**: everything a tool needs — argument names, types, and defaults, the return shape (scalar, `SETOF` composite, `RETURNS TABLE`, OUT params, void, procedures), enum values, and volatility — comes from `pg_proc`/`pg_type`. Arguments with a `DEFAULT` are optional in the tool's input schema; calls that omit them use named-argument notation so PostgreSQL applies the defaults.
- **Skipped functions**: overloaded `@api` names, `VARIADIC` or pseudo-type (polymorphic) arguments, `RETURNS record` without OUT params, and procedures with OUT params are skipped with a logged warning — one exotic function never takes down the rest of the tool surface. Domains resolve to their base type; arrays and composite/range types are supported (the latter via their text form).
