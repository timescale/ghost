...
- **Tool naming**: on the authoring server, generated tool names are prefixed with the snake_cased database name (`my_db_get_user`); prefix collisions (including the reserved `ghost`/`ghost_*` namespace) are disambiguated with an ID-derived suffix. The serving mode exposes a single database's tools and nothing else, so it registers bare function names.
