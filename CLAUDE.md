...
Experimental (gated behind `GHOST_EXPERIMENTAL`): every Ghost database can define custom MCP tools by marking Postgres functions with an `@api` comment — the first line of the `COMMENT ON FUNCTION` text is `@api` (an optional `@api(group, ...)` list is accepted and reserved for future agent scoping), and the remaining lines become the tool's description. Because the definition lives in the database itself, tools are shared across the space and travel with forks and shares.

Key mechanics (see `internal/mcp/function`):

- **Catalog introspection, no SQL parsing**: everything a tool needs — argument names, types, and defaults, the return shape (scalar, `SETOF` composite, `RETURNS TABLE`, OUT params, void), enum values, and volatility — comes from `pg_proc`/`pg_type`. Arguments with a `DEFAULT` are optional in the tool's input schema; calls that omit them use named-argument notation so PostgreSQL applies the defaults.
- **Skipped functions**: overloaded `@api` names, procedures (a void-returning function covers the same ground), `VARIADIC` or pseudo-type (polymorphic) arguments, and `RETURNS record` without OUT params are skipped with a logged warning — one exotic function never takes down the rest of the tool surface. Domains resolve to their base type; arrays and composite/range types are supported (the latter via their text form).
...
- **Two serving modes**: the regular `ghost mcp start` (authoring) registers the `ghost_mcp_tool_refresh` management tool plus a startup snapshot of every database's function tools, introspected concurrently before the server starts serving; `ghost mcp start --serve <database_ref>` (consumer) exposes *only* that database's generated tools. Capability enumeration (`ghost mcp list`/`get`) constructs the server without `Options.FunctionTools`, so it registers the management tool but never connects to any databases.
...
- **Read-only**: generated function tools deliberately ignore the `read_only` config option (marking a function `@api` is an intentional act; the volatility-derived annotations tell clients which tools write).
