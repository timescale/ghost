...
- **Tool naming**: on the authoring server, generated tool names are prefixed with the snake_cased database name (`my_db_get_user`). A function's own schema is folded in too, unless it's `public`, so same-named functions in different schemas of one database don't collide (`my_db_reporting_get_user`). Database names are unique within a space, so a prefix collision only arises when two names differ solely by case or separator style once normalized, or when a name's normalized form lands in the reserved `ghost`/`ghost_*` namespace; either case is disambiguated with an ID-derived suffix, computed against whatever prefixes are already registered (not a fresh listing of every database in the space, which could come back in a different order than the original startup snapshot). The serving mode exposes a single database's tools and nothing else, so it skips the database prefix (schema-qualification still applies).
