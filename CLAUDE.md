...
- **Catalog introspection, no SQL parsing**: argument names, types, defaults, return shape (scalar, `SETOF` composite, `RETURNS TABLE`, OUT params, void), enum values, and volatility all come from `pg_proc`/`pg_type`. Arguments with a `DEFAULT` are optional in the tool's input schema; calls that omit them use named-argument notation so PostgreSQL applies the defaults. Arguments reject `NULL` unless the function declares `DEFAULT NULL`, which is surfaced as optional *and* nullable. Because a tool's schema is only known once its function has been introspected, tools are registered through the SDK's low-level `Server.AddTool` rather than the generic `AddTool[In, Out]`; each tool's handler validates its own input schema on every call.
- **Skipped functions**: procedures, aggregate and window functions, `VARIADIC` or pseudo-type (polymorphic) arguments, and `RETURNS record` without OUT params are skipped with a logged warning, not an error — one exotic function never takes down the rest of the tool surface. Domains resolve to their base type; arrays and composite/range types are supported.
- **Overloaded functions**: same-named `@mcp` functions each become their own tool, distinguished only by the ordinary de-duplication suffix (`billing__get_user`, `billing__get_user_2`). `buildCall` casts every argument placeholder to its declared type so PostgreSQL resolves the call to the intended overload; overloads it still can't disambiguate (e.g. an arg-count overlap from defaults) fail at call time rather than being skipped up front.
- **Tool naming**: a tool's name joins the normalized database name and function name with `__` (`billing__get_user`). Normalization restricts names to `[a-zA-Z0-9_-]`, narrower than the MCP spec's own recommendation, since some clients pass tool names straight through to their own model API without sanitizing them first (e.g. the Claude API's tool-name pattern rejects a dot). The function's own Postgres schema plays no part in the name — same-named functions in different schemas are treated as an ordinary, common collision, not an edge case — so names are truncated to 128 bytes and de-duplicated with a `_2`, `_3`, ... suffix as needed; nothing is ever dropped for a naming reason. See `manager.go` for the normalization/de-duplication logic and how it stays deterministic across reloads.
- **Two serving modes, two constructors**: the regular authoring server (`NewServer`) takes an `Options.FunctionTools` enum (`FunctionToolsMode`) — `FunctionToolsDisabled`, `FunctionToolsManagementOnly` (registers `ghost_mcp_tool_refresh` but connects to no databases; used by capability enumeration — `ghost mcp list`/`get`, shell completion), or `FunctionToolsEnabled` (introspects and registers every database's function tools at startup; used by `ghost mcp start` without `--serve`). The stripped consumer serving mode is a separate constructor, `NewFunctionToolsServer(ctx, app, logger, databaseRef)`, reached via `ghost mcp start --serve <database_ref>`: it exposes *only* that database's generated tools, unprefixed, with no management or other Ghost tools. `internal/mcp` has no knowledge of `GHOST_EXPERIMENTAL` itself — every call site in `internal/cmd` picks the constructor and mode from `app.Experimental`.
