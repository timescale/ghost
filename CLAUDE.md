...
- **Skipped functions**: overloaded `@mcp` names, procedures (a void-returning function covers the same ground), `VARIADIC` or pseudo-type (polymorphic) arguments, and `RETURNS record` without OUT params are skipped with a logged warning — one exotic function never takes down the rest of the tool surface. Domains resolve to their base type; arrays and composite/range types are supported (the latter via their text form).
